...
#undef CONCAT
{
    ...
    meta->style_flags = style == DWRITE_FONT_STYLE_NORMAL ? 0 : FT_STYLE_FLAG_ITALIC;
    ...
    if (!FAILED(hr)) {
        if (exists && size >= 64) {
            uint16_t fsSelection = ((unsigned)data[62] << 8) | (unsigned)data[63];
            meta->style_flags = ass_fsSelection_to_style_flags(fsSelection);
        }
